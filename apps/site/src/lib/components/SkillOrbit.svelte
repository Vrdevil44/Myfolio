...
<div class="grid gap-4 lg:grid-cols-[1.2fr_1fr]">
  <div
    class="relative overflow-hidden rounded-2xl border border-[color:var(--border)] bg-gradient-to-br from-[color:var(--bg-elevated)] to-[color:var(--bg-primary)] p-6"
  >
    <div
      class="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(103,232,249,0.18),transparent_35%)]"
    ></div>
    <div
      class="absolute inset-0 bg-[radial-gradient(circle_at_80%_60%,rgba(124,58,237,0.18),transparent_30%)]"
    ></div>
        ...
        {#each skills as category, index}
          ...
          >
            <div class="node-face">
              ...
              <div
                class="mt-2 flex flex-wrap gap-1 text-[10px] text-[color:var(--text-muted)]"
              >
  ...
  >
    <p class="text-sm text-[color:var(--text-muted)]">
      Use the orbit as a tactile map of Vibhu's toolchain. Drag to rotate and
      explore each cluster; the orbit keeps moving even when you pause. A
      plaintext list remains available for accessibility.
    </p>
    <ul class="mt-4 space-y-3 text-sm text-[color:var(--text-primary)]">
      {#each skills as category}
        <li
          class="rounded-xl border border-[color:var(--border)] bg-[color:var(--bg-primary)]/70 p-3"
        >
          <div class="flex items-center justify-between">
            <span class="font-semibold">{category.name}</span>
            <span class="text-[color:var(--accent-2)]"
              >{Math.round((category.intensity ?? 0.5) * 100)}%</span
            >
          </div>
...
<style>
  .skill-node {
    ...
    transform: translate(-50%, -50%)
      rotateY(calc(var(--index) * (360deg / var(--count))))
  ...
  .skill-node:hover {
    transform: translate(-50%, -50%)
      rotateY(calc(var(--index) * (360deg / var(--count))))
      translateZ(calc(var(--radius) + 16px));
  }
  ...
  .node-face {
    ...
    transform: rotateY(calc(var(--rotation, 0deg)))
      translateZ(calc((var(--intensity) + 0.4) * 40px));
  }
</style>
