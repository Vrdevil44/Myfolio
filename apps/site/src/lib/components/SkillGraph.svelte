...
<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
  {#each skills as skill, index}
    <div
      class="rounded-2xl border border-[color:var(--border)] bg-[color:var(--bg-elevated)] p-4 transition hover:-translate-y-1 hover:border-[color:var(--accent-2)]"
    >
      <div
        class="flex items-center justify-between text-sm font-semibold text-[color:var(--text-primary)]"
      >
      ...
      <div
        class="mt-2 h-2 overflow-hidden rounded-full bg-[color:var(--bg-primary)]"
      >
        <div
