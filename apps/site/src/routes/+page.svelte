<script lang="ts">
  ...
  import RiveSignature from '$lib/components/RiveSignature.svelte';
  import Section from '$lib/components/Section.svelte';
  import SkillGraph from '$lib/components/SkillGraph.svelte';
  import SkillOrbit from '$lib/components/SkillOrbit.svelte';
  ...
  let particleSketch: p5 | null = null;
  let focusedRole = '';
  ...
  onMount(() => {
    ...
    return () => {
      ...
      if (particleSketch) particleSketch.remove();
    };
  });
...
<div class="mx-auto flex w-full max-w-6xl flex-col gap-10 px-5 py-12 lg:gap-14 lg:px-8" id="hero">
  ...
  {#if selectedProject}
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-6" role="dialog" aria-label="Project details">
      <div class="w-full max-w-2xl rounded-2xl border border-[color:var(--border)] bg-[color:var(--bg-primary)] p-6 shadow-2xl">
        <div class="flex items-center justify-between gap-3">
          <div>
            ...
            <h3 class="text-2xl font-bold text-[color:var(--text-primary)]">{selectedProject.name}</h3>
          </div>
          <button
  ...
  >
    <SkillOrbit skills={resume.skills} />
    <div class="mt-4">
